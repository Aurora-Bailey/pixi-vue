...
export default {
  ...
  created () {
    ...
    this.container = new this.$pixi.PIXIWrapper.PIXI.Container()
    ...
    this.$pixi.EventBus.$on('ready', () => {
      if (this.$parent.container) {
        ...
        this.$pixi.PIXIWrapper.PIXIApp.stage.addChild(this.container)
      }

      // Emit a Vue event on every tick with the container and tick delta for an easy way to do frame-by-frame animation.
      // (Not performant)
      this.$pixi.PIXIWrapper.PIXIApp.ticker.add(delta => this.$emit('tick', this.container, delta))
    })
  }
}
</script>
